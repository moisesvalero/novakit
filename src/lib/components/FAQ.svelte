...
<section class="cta-section">
	...
	<div class="cta-inner" use:reveal={{ stage: 'content', delay: 60 }}>
		...
		<h2 class="cta-title cta-assemble" use:reveal={{ stage: 'content', delay: 170 }}>
			{$t('faqCta.titleLine1')}<br /><span class="cta-gradient">{$t('faqCta.titleLine2')}</span>
		</h2>
		<p class="cta-subtitle cta-assemble" use:reveal={{ stage: 'content', delay: 230 }}>
			{$t('faqCta.subtitle')}
		</p>
...
<style>
	.cta-section {
		...
		overflow: hidden;
		padding: 120px 2rem;
		background: linear-gradient(160deg, #fafafa 0%, #f0ebff 40%, #e8f0ff 70%, #f5f0ff 100%);
		text-align: center;
	}
	.cta-orb1 {
		position: absolute;
	...
	.cta-orb2 {
		...
		height: 320px;
		border-radius: 50%;
		background: radial-gradient(circle, rgba(99, 130, 246, 0.1) 0%, transparent 70%);
		filter: blur(50px);
		pointer-events: none;
